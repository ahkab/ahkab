...
"""This module implements the Implicit Euler (IE, aka Backward Euler) and a
first-order prediction formula (FF)
...
def get_required_values():
    ...
    The first tuple is the one to be considered if no FF is needed, the second
    if the FF is also required.
    ...
    :math:`k`, the DF method needs all the :math:`x_{n-i}` values of :math:`x`,
    where :math:`i \le k` (the value x has :math:`i+1` steps before the one we
    will ask for the derivative).  The same applies to ``max_order_of_dx``, but
    it regards :math:`dx/dt` instead of :math:`x`.
    ...
    **Returns:**
    ...
    In the case of the BE formula, this method will return:
