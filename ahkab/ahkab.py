...
def new_dc(
    start, stop, points, source, sweep_type='LINEAR', guess=True, x0=None, outfile=None,
        verbose=0):
...
def new_tran(
    tstart, tstop, tstep, x0='op', method=transient.TRAP, use_step_control=True,
...
def new_ac(start, stop, points, x0='op', sweep_type='LOG', outfile=None, verbose=0):
    ...
    if outfile is None or outfile == 'stdout':
        ...
        outfile += '.ac'
    return {
        'type': 'ac', 'start': start, 'stop': stop, 'points': points, 'sweep_type': sweep_type,
        'x0': x0, 'outfile': outfile, 'verbose': verbose}


def new_pss(
    period, x0=None, points=None, method='brute-force', autonomous=False, outfile=None,
        verbose=0):
