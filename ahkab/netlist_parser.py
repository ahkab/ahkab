...
def main_netlist_parser(circ, netlist_lines, subckts_dict, models):
    ...
    parse_function = {
        ...
        'f': lambda line, line_elements: parse_elem_cccs(line, circ, line_elements),
        'h': lambda line, line_elements: parse_elem_ccvs(line, circ, line_elements),
...
def parse_elem_ccvs(line, circ, line_elements=None):
    """Parses a current-controlled voltage source (ccvs) from the line supplied,
    adds its nodes to the circuit instance ``circ`` and returns a list holding
    the ``HVSource`` element.

    **Parameters:**

    line : string
        The netlist line, if you have already splitted it, set this to
        ``None`` and supply the elements through ``line_elements``.
    circ : circuit instance.
        The circuit to which the element will be added.
    line_elements : list of strings, optional
        They will be generated by the function from ``line.split()`` if set to
        ``None``.
...
def parse_elem_cccs(line, circ, line_elements=None):
